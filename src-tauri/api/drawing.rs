use crate::drawing_engine::{DrawStroke, DrawingEngine, Vertex2D};
...
pub async fn initialize_drawing_engine(state: State<'_, DrawingState>) -> Result<String, String> {
...
pub async fn create_drawing_layer(
    ...
    debug!(
        "[Drawing API] 引数受信確認 - layer_id: '{}', width: {}, height: {}",
    ...
    if width > 4096 || height > 4096 {
        error!(
            "[Drawing API] 解像度上限超過: {}x{} (最大: 4096x4096)",
            width, height
        );
    ...
    // 重複レイヤーチェック
    {
        ...
        if layers_guard.contains_key(&layer_id) {
            warn!(
                "[Drawing API] レイヤーID重複: {} - 既存レイヤーを上書き",
                layer_id
            );
        } else {
...
pub async fn draw_line_on_layer(
    ...
    let (layer_width, layer_height) = {
        ...
        match layers_guard.get(&layer_id) {
            Some(dimensions) => {
                debug!(
                    "[Drawing API] レイヤー確認OK: {} ({}x{})",
                    layer_id, dimensions.0, dimensions.1
                );
                dimensions.clone()
            }
            None => {
...
pub async fn draw_stroke_on_layer(
    ...
    // ストロークを描画
    {
        ...
        let vertex_points: Vec<Vertex2D> = points
            .iter()
            .map(|p| {
        ...
        // ストロークを描画
        engine
            .draw_stroke_to_layer(&layer_id, &stroke)
...
#[tauri::command]
#[cfg_attr(feature = "specta", specta::specta)]
pub async fn clear_layer(layer_id: String, state: State<'_, DrawingState>) -> Result<(), String> {
    ...
    // レイヤーをクリア（透明）
    {
        ...
        engine
            .clear_layer_texture(&layer_id, Some(wgpu::Color::TRANSPARENT))
...
#[tauri::command]
#[cfg_attr(feature = "specta", specta::specta)]
pub async fn remove_layer(layer_id: String, state: State<'_, DrawingState>) -> Result<(), String> {
...
pub async fn begin_realtime_stroke(
    ...
    debug!(
        "[Drawing API] リアルタイムストローク開始: layer={}, tool={}",
        layer_id, tool
    );
...
pub async fn add_realtime_stroke_point(
    ...
    trace!(
        "[Drawing API] ストローク点追加: stroke={}, point=({}, {})",
        stroke_id,
        point.x,
        point.y
    );
    ...
    if should_update {
        ...
        let partial_data = {
            ...
            if recent_points.len() >= 2 {
                ...
                engine
                    .draw_stroke_to_layer(&layer_id, &partial_stroke)
...
pub async fn complete_realtime_stroke(
    ...
    info!(
        "[Drawing API] リアルタイムストローク完了: id={}, points={}",
        stroke_id,
        stroke_data.points.len()
    );
    Ok(())
}
...
pub struct DrawingStats {
    pub layers_count: u32,  // usizeからu32に変更（JavaScript Number互換）
    pub memory_used: f64,  // u64からf64に変更（JavaScript Number互換）
    pub memory_limit: f64,  // u64からf64に変更（JavaScript Number互換）
    pub active_textures: u32,  // usizeからu32に変更（JavaScript Number互換）
    pub total_textures: u32,  // usizeからu32に変更（JavaScript Number互換）
}

#[tauri::command]
#[cfg_attr(feature = "specta", specta::specta)]
pub async fn get_drawing_stats(state: State<'_, DrawingState>) -> Result<DrawingStats, String> {
    let layers_count = {
        let layers_guard = state.layers.lock().await;
        layers_guard.len()
    };
    ...
    Ok(DrawingStats {
        layers_count: layers_count as u32,
        memory_used: memory_used as f64,
        memory_limit: memory_limit as f64,
        active_textures: active_textures as u32,
        total_textures: total_textures as u32,
    })
}
...
#[tauri::command]
#[cfg_attr(feature = "specta", specta::specta)]
pub async fn cleanup_textures(state: State<'_, DrawingState>) -> Result<String, String> {
...
pub async fn get_detailed_engine_state(
    ...
    debug!(
        "[Drawing API] 詳細エンジン状態: エンジン初期化={}, レイヤー数={}, メモリ使用量={}",
        state_info.engine_initialized,
        state_info.layers.len(),
        state_info.memory_used
    );
...
#[tauri::command]
#[cfg_attr(feature = "specta", specta::specta)]
pub async fn get_all_layers_info(state: State<'_, DrawingState>) -> Result<Vec<LayerInfo>, String> {
    ...
    for (layer_id, width, height) in layer_ids {
        ...
        debug!(
            "[Drawing API] レイヤー情報: {} ({}x{}) エンジン存在={}",
            layer_id, width, height, exists_in_engine
        );
    }

    info!(
        "[Drawing API] 全レイヤー情報取得完了: {} レイヤー",
        layer_infos.len()
    );
    Ok(layer_infos)
}
...
pub struct SystemMemoryInfo {
    pub process_memory_mb: f64,  // u64からf64に変更（JavaScript Number互換）
    pub available_memory_mb: f64,  // u64からf64に変更（JavaScript Number互換）
    pub texture_memory_mb: f64,  // u64からf64に変更（JavaScript Number互換）
}
...
pub async fn get_system_memory_info(
    ...
    let texture_memory_mb = {
        ...
        match engine_guard.as_ref() {
            Some(engine) => {
                let (used, _limit, _active, _total) =
                    engine.get_texture_memory_stats().unwrap_or((0, 0, 0, 0));
    ...
    let memory_info = SystemMemoryInfo {
        process_memory_mb: 0.0,   // 将来実装
        available_memory_mb: 0.0, // 将来実装
        texture_memory_mb: texture_memory_mb as f64,
    };

    debug!(
        "[Drawing API] システムメモリ情報: テクスチャメモリ={}MB",
        memory_info.texture_memory_mb
    );
...
#[tauri::command]
#[cfg_attr(feature = "specta", specta::specta)]
pub async fn log_detailed_state(state: State<'_, DrawingState>) -> Result<(), String> {
