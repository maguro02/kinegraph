...
use crate::drawing_engine::{DrawStroke, DrawingEngine};
...
pub async fn create_project(
    ...
    debug!(
        "[API] プロジェクトパラメータ: name={}, width={}, height={}, frame_rate={}",
    ...
    let mut engine = match engine_arc.try_lock() {
        ...
        Err(e) => {
            warn!(
                "[API] DrawingEngine ミューテックスロック取得失敗、await で再試行: {}",
                e
            );
...
pub async fn get_system_info() -> Result<String, String> {
    ...
    let info = format!("Drawing app initialized - Tauri + wgpu + React + jotai");
...
pub async fn draw_stroke(
    ...
    for point in args.points {
        let norm_pos = engine
            .screen_to_normalized((point.x, point.y), (args.canvas_width, args.canvas_height));
