...
function useCanvasDrawing(layerId: string | null, width: number, height: number) {
    ...
    // リアルタイム描画エンジンフック
    const { beginStroke, addStrokePoint, completeStroke } = useDrawingEngine(
    ...
    // 線描画をRustエンジンに送信
    const drawLineToEngine = useCallback(
    ...
    const startDrawing = useCallback(
        async (e: MouseEvent<HTMLCanvasElement>) => {
            ...
            try {
                const rgba = hexToRgba(color);
                await beginStroke(rgba, brushSize, currentTool);
            } catch (err) {
                console.error("[Canvas] ストローク開始エラー:", err);
            }

            console.log("[Canvas] 描画開始:", { x, y, tool: currentTool });
        },
        [layerId, currentTool, color, brushSize, beginStroke]
    );
    ...
    const draw = useCallback(
        (e: MouseEvent<HTMLCanvasElement>) => {
            ...
            // リアルタイムストローク点を追加（非同期でRustエンジンに送信）
            addStrokePoint(x, y, 1.0).catch((err) => {
                console.error("[Canvas] ストローク点追加エラー:", err);
            });

            // 一時的なプレビュー描画（Rustからの更新が来るまでの間）
        ...
        [layerId, currentTool, color, brushSize, addStrokePoint]
    );
    ...
    const stopDrawing = useCallback(async () => {
        ...
        console.log("[Canvas] 描画終了: ストローク点数 =", strokePoints.current.length);
...
export function Canvas({ width, height }: CanvasProps) {
    const { isInitialized, isInitializing, error, initialize } = useRustDrawingEngine();
    ...
    const { canvasRef, startDrawing, draw, stopDrawing, updateCanvasFromLayer } = useCanvasDrawing(
        activeLayerId,
        width,
        height
    );
