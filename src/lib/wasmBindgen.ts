...
// wasm-bindgenで生成されたモジュールを直接インポート
import init, { DrawingContext as WasmDrawingContext, SharedBuffer, check_webgpu_support } from "../../src-wasm/pkg/kinegraph_wasm.js";
...
async function initializeWasmModule(): Promise<void> {
    ...
    wasmInitPromise = (async () => {
        try {
            // wasm-bindgenの初期化関数を呼び出す
            // WASMファイルのパスを指定（Viteがビルド時に処理）
            await init();
            
            wasmInitialized = true;
            console.log("WASM module initialized successfully with wasm-bindgen");
        } catch (error) {
...
export class WasmBindgenDrawingContext implements DrawingContext {
    private drawingContext: WasmDrawingContext | null = null;
    ...
    async initialize(): Promise<void> {
        ...
        // DrawingContextを作成 (newはstatic methodなので await が必要)
        this.drawingContext = await WasmDrawingContext.new(this.width, this.height);

        // SharedArrayBufferが利用可能な場合は、SharedBufferを作成
        if (typeof SharedArrayBuffer !== "undefined") {
            try {
                this.sharedBuffer = new SharedBuffer(this.width * this.height * 4);
...
export async function createWasmBindgenContext(
    canvas: HTMLCanvasElement,
    width: number,
    height: number
): Promise<DrawingContext> {
    const context = new WasmBindgenDrawingContext(canvas, width, height);
