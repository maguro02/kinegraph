...
// @ts-ignore - This function is generated by tauri-specta and may not be used
...
// 手動追加: バイナリ通信用コマンド（Request型を使用するため自動生成されない）
export const binaryCommands = {
	async drawBinary(data: Uint8Array): Promise<Uint8Array> {
		// Tauri v2では、Request型を使うハンドラーにはバイナリデータを直接渡す
		// invokeの第2引数にUint8Arrayを渡すと、Rust側でRequest<'_>として受け取れる
		return await TAURI_INVOKE("draw_binary", data);
	},
	async drawBatchBinary(data: Uint8Array): Promise<Uint8Array> {
		return await TAURI_INVOKE("draw_batch_binary", data);
	},
	async getCanvasDataBinary(canvasId: string, compressed: boolean): Promise<Uint8Array> {
		return await TAURI_INVOKE("get_canvas_data_binary", { canvasId, compressed });
	},
	async getRenderResultBinary(canvasId: string, compressed: boolean): Promise<Uint8Array> {
		return await TAURI_INVOKE("get_render_result_binary", { canvasId, compressed });
	},
	async getCanvasDataBinaryStream(canvasId: string, options: StreamingOptions): Promise<Uint8Array> {
		return await TAURI_INVOKE("get_canvas_data_binary_stream", { canvasId, options });
	},
	async getCanvasDataChunked(canvasId: string, chunkSize: number, compress: boolean): Promise<[ImageHeader, ImageChunk[]]> {
		return await TAURI_INVOKE("get_canvas_data_chunked", { canvasId, chunkSize, compress });
	},
	async getCanvasDataPerfTest(canvasId: string, options: PerformanceTestOptions): Promise<Uint8Array> {
		return await TAURI_INVOKE("get_canvas_data_perf_test", { canvasId, options });
	}
}

// バイナリ通信用の追加型定義（自動生成されない場合の一時的な定義）
...
// シンプルバイナリプロトコル用のコマンド（手動定義）
export const simpleBinaryCommands = {
	async drawCommand(data: Uint8Array): Promise<void> {
		return await TAURI_INVOKE("draw_command_simple_binary", data);
	},
	
	async drawBatch(data: Uint8Array): Promise<void> {
		return await TAURI_INVOKE("draw_batch_simple_binary", data);
	},
}
